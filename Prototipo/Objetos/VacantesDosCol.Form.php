...
require_once "Clases/FormularioDosCol.php";
...
****/	
	...
	$f = new FormularioDosCol('celda','formuid', '90%');
//Asignaciones	
	$f->SetBorde('1');
	$f->SetPie('Nota al pie', 'pieform','idpie' );
	$f->SetEncabezado('Ingreso de vacantes', 'principal','idhead' );

	$f->SetColIzq($L1->GetHTML()." ".$I1->GetHTML(), '','' );
	$f->SetColDer($L2->GetHTML()." ".$I2->GetHTML(), '','' );
	
	$f->SetColDer($LTorr->GetHTML()." ".$ITorr->GetHTML()." ".
				   $LManz->GetHTML()." ".$IManz->GetHTML()." ".
				   $LSec->GetHTML()." ".$ISec->GetHTML()." ".
				   $LCP->GetHTML()." ".$ICP->GetHTML(), '','' );
	
	$f->SetColDer($IBoton1->GetHTML(), '','' );
	$f->SetColIzq($IBoton2->GetHTML(), '','' );	
	
//Mostrar Controles	
	//$f->test();
	//echo "vamos... <br> ".$f->GetB();	
	echo $f->Draw();
